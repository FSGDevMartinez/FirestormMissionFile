...
if(!(getNumber(missionConfigFile >> "DebugMode") isEqualTo 1)) then {
    ...
    if(!_IsAdmin && {_ServerName != "FireStormGaming | Lakeside Valley | Malden Wasteland"}) then {_notOn = true;};
    if(!_IsAdmin && {!(_ChannelName in _WhiteListedChannels)}) then {_notOn = true;};
    if(!_IsAdmin && {!_isTeamSpeakPluginEnabled}) then {_notOn = true;};

    if(_notOn) then {
        cutText["Wrong TFR Channel/Server TFR Plugin Disabled. Ensure you are on TS:74.91.121.206","BLACK FADED"];
    } else {
        cutText ["","PLAIN"];
    };
    ...
    if(side player isEqualTo civilian) then {
        ...
        if(count _radios < 2) then {
            ...
            if(_wtflol) then {
                ...
                if( _curfreqlr > 33 && _curfreqlr < 35 ) then {
                    hintSilent "33.1 - 34.9 are blocked frequencies.";
            ...
            if(_Wtfno) then {
                ...
                if(!isNil "_curfreqsr") then {
                    if( _curfreqsr > 33 && _curfreqsr < 35 ) then {
                        hintSilent "33.1 - 34.9 are blocked frequencies.";
