...
if(player getvariable "restrained" || player getvariable "tied") exitwith {hint "You're restrained, you can't do that right now!"};
...
if(isPlayer _curTarget && _curTarget isKindOf "Man" && !(player getVariable "restrained")) then {
		...
		if((typeOf _curTarget) in _animalTypes) then {
			...
			if((typeOf _curTarget) in _miscItems) then {
				...
				if((typeOf _curTarget) == _money && {!(_curTarget getVariable["inUse",false])}) then {
					...
					if ((typeOf _curTarget) == "Land_CargoBox_V1_F") then {
						_handle = [_curTarget] spawn life_fnc_lootTreasure;
						waitUntil {scriptDone _handle};
					};
				};
			};
